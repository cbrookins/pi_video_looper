...
class VLC:
    ...
    def _load_config(self, config):
        self._extensions = config.get('vlc', 'extensions') \
                                 .translate(str.maketrans('', '', ' \t\r\n.')) \
                                 .split(',')
        self._extra_args = config.get('vlc', 'extra_args').split()
        self._sound = config.get('vlc', 'sound').lower()
        assert self._sound in ('hdmi', 'local', 'both', 'alsa'), 'Unknown vlc sound configuration value: {0} Expected hdmi, local, both or alsa.'.format(self._sound)
